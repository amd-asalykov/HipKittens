...
template<int D> struct attn_bwd_combined_globals { 
    gl<bf16, -1, -1, -1, -1> Q, K, V, O, dOg;
    gl<float, -1, -1, -1, -1> dQg, dKg, dVg;
...
__device__ inline static void atomic_store_accum_row(const GL &dst, const RT &src, const COORD &idx, const int warp_id) { 
    using T = base_types::packing<typename RT::dtype>::unpacked_type;
    using U = typename GL::dtype;
    ...
    int col_offset = laneid/(src.tile_size_row) * 4 + warp_id * 16;
    ...
    for(int i = 0; i < src.height; i++) {
        ...
        for(int j = 0; j < src.width; j++) {
            ...
            uint32_t byte_offset_1 = static_cast<uint32_t>((row * row_stride + col + 1) * sizeof(U));
            uint32_t byte_offset_2 = static_cast<uint32_t>((row * row_stride + col + 2) * sizeof(U));
            uint32_t byte_offset_3 = static_cast<uint32_t>((row * row_stride + col + 3) * sizeof(U));
...
template<int axis, ducks::rt::accumulator_row_layout RT, ducks::gl::all GL, ducks::coord::tile COORD=coord<RT>>
__device__ inline static void atomic_store_accum_row_bf16(const GL &dst, const RT &src, const COORD &idx, const int warp_id) { 
    using T = base_types::packing<typename RT::dtype>::unpacked_type;
    using U = typename GL::dtype;
...
template<int axis, ducks::rt::accumulator_row_layout RT, ducks::gl::all GL, ducks::coord::tile COORD=coord<RT>>
__device__ inline static void atomic_add_fp32_with_warpid(const GL &dst, const RT &src, const COORD &idx, int warpid) { 
    using T = base_types::packing<typename RT::dtype>::unpacked_type;
    using T2 = base_types::packing<typename RT::dtype>::packed_type;
    using U = typename GL::dtype;
    using U2 = base_types::packing<U>::packed_type;

    static_assert(std::is_same_v<U, float>, "atomic_add_fp32 is only supported for float");
    ...
    for(int i = 0; i < src.height; i++) {
        ...
        for(int j = 0; j < src.width; j++) {
            ...
            // uint32_t byte_offset_3 = static_cast<uint32_t>((row * row_stride + col + 3) * sizeof(U));
            uint32_t byte_offset_0 = static_cast<uint32_t>((tile_offset + lane_offset) * sizeof(U));
            uint32_t byte_offset_1 = static_cast<uint32_t>((tile_offset + lane_offset + 1) * sizeof(U));
            uint32_t byte_offset_2 = static_cast<uint32_t>((tile_offset + lane_offset + 2) * sizeof(U));
            uint32_t byte_offset_3 = static_cast<uint32_t>((tile_offset + lane_offset + 3) * sizeof(U));
...
__device__ inline static void load_shuffled(RT &dst, const GL &src, const COORD &idx) {
    ...
    static_assert(std::is_same_v<U, bf16> || std::is_same_v<U, float>, "load_shuffled is only supported for bf16 or float");
    ...
    for(int i = 0; i < dst.height; i++) {
        ...
        for(int j = 0; j < dst.width; j++) {
            ...
            if constexpr (std::is_same_v<U, bf16>) {
...
__global__ void attend_bwd_combined_ker(const attn_bwd_combined_globals<D> g) {
    ...
    for (int i = 0; i < num_steps - 1; ++i, tic ^= 1, toc ^= 1) {
        ...
        // dot slice 0
        {
            ...
            // atomic_add_fp32_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, q_seq_idx * 4, 0}, warpid);
            __builtin_amdgcn_sched_barrier(0);
            atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4, q_head_idx, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        // dot slice 1
        {
            ...
            // atomic_add_fp32_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, q_seq_idx * 4 + 1, 0}, warpid);
            __builtin_amdgcn_sched_barrier(0);
            atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4 + 1, q_head_idx, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        // dot slice 2
        {
            ...
            // atomic_add_fp32_with_warpid<2>(g.dQg, dQ_i, {batch_idx, q_head_idx, q_seq_idx * 4 + 2, 0}, warpid);
            __builtin_amdgcn_sched_barrier(0);
            atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4 + 2, q_head_idx, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
        // dot slice 3
        {
            ...
            __builtin_amdgcn_sched_barrier(0);
            atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4 + 3, q_head_idx, 0}, warpid);
            __builtin_amdgcn_s_setprio(0);
            __builtin_amdgcn_s_barrier();
            __builtin_amdgcn_sched_barrier(0);
        }
    ...
    // dot slice 0
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        // atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4, 0}, warpid);
        // atomic_add_fp32_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4, 0}, warpid);
        __builtin_amdgcn_sched_barrier(0);
        atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4, q_head_idx, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
    ...
    // dot slice 1
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        // atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4 + 1, 0}, warpid);
        // atomic_add_fp32_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4 + 1, 0}, warpid);
        __builtin_amdgcn_sched_barrier(0);
        atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4 + 1, q_head_idx, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
    ...
    // dot slice 2
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        // atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4 + 2, 0}, warpid);
        // atomic_add_fp32_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4 + 2, 0}, warpid);
        __builtin_amdgcn_sched_barrier(0);
        atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4 + 2, q_head_idx, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
    ...
    // dot slice 3
    {
        ...
        mma_AtB(dQ_i_T, K_j_col, dP_ij_bf16_col_T,  dQ_i_T);
        swap_layout_and_transpose(dQ_i, dQ_i_T);
        // atomic_pk_add_bf16_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4 + 3, 0}, warpid);
        // atomic_add_fp32_with_warpid<2>(g.dQg, dQ_i, {batch_idx, first_q_head + GROUP_SIZE - 1, (num_steps_per_head - 1) * 4 + 3, 0}, warpid);
        __builtin_amdgcn_sched_barrier(0);
        atomic_store_accum_row<1>(g.dQg, dQ_i, {batch_idx, q_seq_idx * 4 + 3, q_head_idx, 0}, warpid);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_s_barrier();
...
template<int D> struct attn_dq_shuffle_globals { 
    gl<float, -1, -1, -1, -1> dQg_in, dQg_out;
    dim3 grid() { return dim3(ATTN_B, ATTN_H_Q, ATTN_N / (DOT_SLICE_QO * NUM_WARPS)); }
    dim3 block() { return dim3(NUM_THREADS); }
    size_t dynamic_shared_memory() { return MAX_SHARED_MEMORY; }
};
...
__global__ void attend_dq_shuffle_ker(const attn_dq_shuffle_globals<D> g) {
    ...
    qo_tile<D, float, accum_row_l> dQg;
...
PYBIND11_MODULE(tk_kernel_bkwd_fp32, m) {
